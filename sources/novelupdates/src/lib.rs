...
impl NovelUpdatesSource {
    ...
    fn parse_novel_list(doc: &Html, base_url: &str) -> Result<Vec<SearchResultDto>, String> {
        let item_sel = Selector::parse("div.search_main_box_nu").map_err(|e| e.to_string())?;
        let title_sel = Selector::parse(".search_title > a").map_err(|e| e.to_string())?;
        let cover_sel = Selector::parse(".search_img_nu img").map_err(|e| e.to_string())?;
